...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> {
  ...
  void initState() {
    super.initState();
    // Prendiamo lo stato iniziale
    isEmailVerified = FirebaseAuth.instance.currentUser?.emailVerified ?? false;

    if (!isEmailVerified) {
      sendVerificationEmail();

      // Controlla ogni 3 secondi se l'utente ha cliccato il link
  ...
  Future<void> checkEmailVerified() async {
    // IMPORTANTE: ricarica l'utente per aggiornare il valore emailVerified
    ...
    if (isEmailVerified) {
      ...
      if (mounted) {
        ...
        // NELLA SCHERMATA DI VERIFICA EMAIL O LOGIN
      Navigator.pushReplacement(
        context, 
        MaterialPageRoute(builder: (context) => const Structure()) // <--- PUNTA QUI!
      );
      }
    }
  }
  ...
  Future<void> sendVerificationEmail() async {
    try {
      ...
      // Aspetta 30 secondi prima di permettere un nuovo invio
            ...
            child: Stack(
              children: [
                ...
                Positioned(
                  ...
                  right: 20,
                  child: const Text(
                    'Controlla la tua posta e clicca sul link di conferma.\nUna volta fatto, verrai reindirizzato automaticamente.',
                    textAlign: TextAlign.center,
